...
function Dashboard() {
  ...
  var setdaysdiff;
  function handleday(remainderdate ,name){
    ...
    if(setdaysdiff < 7){
      ...
      // expirydate:setdetails.remainderdate}])
    }
    }
  ...
  return (
    <div className='main'>
      ...
      <div>
              ...
              <tbody>
                            {
                                Users.map((data,i) => (
                                    <tr key={i}>
                                        ...
                                        <td onLoad={handleday(data.expirydate,data.Name)}>{setdaysdiff} Days</td>
        ...
        <div>{
          details.map((data,i)=> (<li key={i}>{details.name}</li>))}
          </div>  
      </div>
    </div>
  )
